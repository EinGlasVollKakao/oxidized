...
describe Oxidized::Source::HTTP do
  ...
  describe "#string_navigate" do
    ...
    it "should be able to navigate multilevel-hash" do
      _(Oxidized::Source::HTTP.new.send(:string_navigate, h1, "jotain.2")).must_equal "jotain"
    end
    it "should be able to navigate hash/array combination" do
      _(Oxidized::Source::HTTP.new.send(:string_navigate, h1, "inventory[0].ip")).must_equal "10.10.10.10"
    end
    it "should return nil on non-existing string key" do
      _(Oxidized::Source::HTTP.new.send(:string_navigate, h1, "jotain.3")).must_be_nil
    end
    it "should return nil on non-existing array index" do
      _(Oxidized::Source::HTTP.new.send(:string_navigate, h1, "inventory[3]")).must_be_nil
    end
  end
end
